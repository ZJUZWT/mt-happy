...
export function connectRoutes(app: Fastify) {
    ...
    app.get('/v1/connect/github/callback', {
        ...
        if (!tokenData) {
            ...
            return reply.redirect('https://mt.hk.swannzh.icu?error=invalid_state');
        }
        ...
        if (!clientId || !clientSecret) {
            return reply.redirect('https://mt.hk.swannzh.icu?error=server_config');
        }
        ...
        try {
            ...
            if (tokenResponseData.error) {
                return reply.redirect(`https://mt.hk.swannzh.icu?error=${encodeURIComponent(tokenResponseData.error)}`);
            }
            ...
            if (!userResponse.ok) {
                return reply.redirect('https://mt.hk.swannzh.icu?error=github_user_fetch_failed');
            }
            ...
            return reply.redirect(`https://mt.hk.swannzh.icu?github=connected&user=${encodeURIComponent(userData.login)}`);

        } catch (error) {
            log({ module: 'github-oauth' }, `Error in GitHub GET callback: ${error}`);
            return reply.redirect('https://mt.hk.swannzh.icu?error=server_error');
        }
    });
